...
import { ArrowLeft, RefreshCw, Shield, Eye, EyeOff, Lock, User, Zap } from 'lucide-react';
...
export const Login: React.FC = () => {
    ...
    const handleDemoLogin = async () => {
        ...
        // 1. Try to login
        const loginResult = await storageService.loginAdmin(demoEmail, demoPass);
        
        if (loginResult.success) {
            addToast('Access Granted. Welcome back, Commander.', 'success');
            navigate('/admin/dashboard');
            setLoading(false);
            return;
        }

        // 2. If login fails, try to sign up (first time setup)
        addToast('Initializing strategic access...', 'info');
        const signupResult = await storageService.signUpAdmin(demoEmail, demoPass, demoName);
        
        if (signupResult.success) {
            addToast('Demo profile established. Synchronizing...', 'success');
            // Try login again after signup
            const secondLogin = await storageService.loginAdmin(demoEmail, demoPass);
            if (secondLogin.success) {
                navigate('/admin/dashboard');
            } else {
                addToast('Account created but auth sync failed.', 'info');
            }
        } else {
            const msg = storageService.getErrorMessage(signupResult.error);
            // Specific handling for rate limits
            if (msg.includes('rate limit exceeded')) {
                addToast('Supabase rate limit exceeded. Please wait 1-5 minutes or check Authentication -> Rate Limits in dashboard.', 'error');
                setErrorDetails(`RATE_LIMIT_ERROR: ${msg}`);
            }
            // If it's just "already registered", try login one last time or show error
            else if (msg.includes('already registered')) {
                const retry = await storageService.loginAdmin(demoEmail, demoPass);
                if (retry.success) {
                    navigate('/admin/dashboard');
                } else {
                    addToast('Account exists but password mismatch.', 'error');
                }
            } else {
                    ...
                    <div className="p-8">
                        ...
                        <div className="flex flex-col items-center gap-4 my-6">
                            <div className="flex items-center gap-3 w-full">
                                ...
                                <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Quick Access</span>
                                <div className="flex-1 h-px bg-slate-100 dark:bg-slate-800" />
                            </div>
                            ...
                            <button
                                ...
                                disabled={loading || seeding}
                                className="w-full py-3 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold text-xs uppercase tracking-widest transition-all active:scale-[0.98] border border-slate-200 dark:border-slate-700 flex items-center justify-center gap-2"
                            >
                                <Zap size={14} className="text-maroon-500 fill-maroon-500" />
                                Quick Demo Login
                            </button>
                        </div>
